...
userSchema.methods.generateResetPasswordOTP = function () {
    var otp = crypto_1.default.randomInt(100000, 999999).toString().padStart(6, '0'); // Always 6 digits
...
userSchema.methods.validateResetPasswordOTP = function (otp) {
    if (!this.resetPasswordOTP || !this.otpExpiration)
        return false;
    var otpStored = this.resetPasswordOTP.toString().trim();
    var otpReceived = otp.toString().trim();
